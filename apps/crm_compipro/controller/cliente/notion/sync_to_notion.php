...
try {
    ...
    if ($request === 'POST') {
        ...
        if ($saved['success']) {
            ...
            // error_log("ID USUARIO EN SESION: ".$idusuario);
            if($idusuario){
            ...
            if ($notion_page_id) {
                ...
                $result = $cliente->update(); // Actualizar notion_page_id
                if($result['success']){
                    echo json_encode(['success' => $result['success'], 'message' => 'Cliente sincronizado con Notion']);
                }else{
