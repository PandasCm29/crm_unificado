...
// ======================
// FILTRAR UNICAMENTE POR NÚMERO DE CLIENTE
// ======================
    ...
    $rows = $cliente->getByNumero($num, $limit, $offset);
        ...
        foreach ($rows as $c) {
            ...
            $acciones = '<div data-id="'.htmlspecialchars($idcliente).'" data-propio="'.htmlspecialchars($data_propio).'"class="flex flex-row space-x-2">
                    <button class="p-0 m-0 bg-transparent border-none outline-none hover:bg-gray-100" title="Editar" onclick="abrirModalEditar(\'' . htmlspecialchars($idcliente) . '\')">
...
if ($action === 'filter_por_campo') {
    ...
    $total = $cliente->countByCampo($campo, $valor);
    $rows = $cliente->getByCampo($campo, $valor, $limit, $offset);
...
if ($action === 'buscar_en_todos') {
    $valor  = $_GET['valor'] ?? '';
    $page   = max(1, (int)($_GET['page'] ?? 1));
    $limit  = max(1, (int)($_GET['limit'] ?? 25));
    $offset = ($page - 1) * $limit;

    $total = $cliente->countEnTodos($valor);
    $rows  = $cliente->buscarEnTodos($valor, $limit, $offset);
    ...
    if ($action === 'duplicate') {
        ...
        $cliente->idusuario = (int)($_SESSION['idusuario'] ?? 0);

        if ($cliente->idcliente <= 0 || $cliente->idusuario <= 0) {
            echo json_encode(['success'=>false,'message'=>'Parámetros inválidos (idcliente/idusuario).']);
            exit;
        }

        $resultado = $cliente->duplicarCliente();
        echo json_encode(
        !empty($resultado['success'])
            ? ['success'=>true,'message'=>'Cliente duplicado correctamente.','idcliente'=>$resultado['idcliente'] ?? null]
            : ['success'=>false,'message'=>'Error al duplicar cliente: '.($resultado['message'] ?? 'Error desconocido')]
        );
        exit;
    }
    ...
    if ($seccion === 'perfil') {
        ...
        // Actualizar solo el perfil del cliente
        if ($cliente->updatePerfil()) {
            echo json_encode([
                'success' => true,
            ...
            echo json_encode([
                'success' => false,
                'message' => "Fallo al actualizar perfil: " . ($err[2] ?? 'Error desconocido')
            ]);
        }
        exit;
    }
