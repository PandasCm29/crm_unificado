...
$areas_notion = ["Administracion", "Comercial", "Logistica"];
try {
    ...
    $usuarioNotion->nombres = $usuarioModel->nombres;
    $usuarioNotion->apellidos = $usuarioModel->apellidos;
    $usuarioNotion->area         = $usuarioModel->area;
    $usuarioNotion->puesto         = $usuarioModel->puesto;
    ...
    if (in_array($usuarioNotion->area, $areas_notion, true)) {
    ...
    } else if(isset($data['sincronizar'])){
        ...
        if($method === 'POST'){
            echo json_encode([
                'success' => true,
                'title' => 'Confirmación',
                'message' => '¿Deseas sincronizar este usuario en Notion?',
                'question' => true,
            ]);
            exit;
        }else{
            $data['sincronizar'] = $usuarioModel->notion_page_id ? true:false;
        }
    }
    ...
    if ($saved['success']) {
        if($sincronizar){
            ...
            echo json_encode(['success' => true, 'message' => $saved['message']]);
        }
    } else {
